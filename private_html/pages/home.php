<div class="container-fluid bg-dark text-light wrapper justify-content-center align-items-center">
    <div class="container text-center text">
        <div class="row">
            <h1 style="font-size: clamp(2em, 4em, 8em);">Studio&nbsp;Brox</h1>
            <h2 style="font-size: clamp(1em, 2em, 4em);">Music and Graphics for all your needs</h2>
            <div class="container mt-4">
                <p class="fs-3">What are you looking for today?</p>
                <div class="container">
                    <div class="row">
                        <div class="col-sm-4 mb-2">
                            <div class="card">
                                <div class="card-body text-center">
                                    <img height="64" src="static/media/svg/image-svgrepo-com.svg" alt="">
                                    <h5 class="card-title text text-dark pt-2">Graphics/Animation</h5>
                                    <a href="#" class="btn btn-success">Go to category</a>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-4 mb-2">
                            <div class="card">
                                <div class="card-body text-center">
                                    <img height="64" src="static/media/svg/note-svgrepo-com.svg" alt="">
                                    <h5 class="card-title text text-dark pt-2">Music/Sound</h5>
                                    <a href="#" class="btn btn-success">Go to category</a>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-4 mb-2">
                            <div class="card">
                                <div class="card-body text-center">
                                    <img height="64" src="static/media/svg/toolbox-svgrepo-com.svg" alt="">
                                    <h5 class="card-title text text-dark pt-2">Miscellaneous</h5>
                                    <a href="#" class="btn btn-success">Go to category</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>